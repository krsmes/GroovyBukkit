...
class GroovyPlugin extends JavaPlugin
{
	...
	void onEnable() {
		...
		globalData.runner = new GroovyRunner(this, null, globalData)
...
//
	...
	void initScripts() {
		new File(GroovyRunner.SCRIPT_LOC+'/startup').eachFileMatch(groovy.io.FileType.FILES, ~/.*\.groovy/) { f ->
			log.info("${description.name} initializing ${f.name}")
			def result = globalData.runner.run(f.text)
			if (result instanceof Map) {
				globalData.runner.register(f.name, result)
			}
		}
	}
